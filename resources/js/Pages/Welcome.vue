...
const handleSubmit = async (type) => { 
    if(form.dni == '') {
      return Swal.fire(
        'OPS',
        'La cedula del empleado es obligatoria',
        'warning'
      )
    }
    form.type = type;

    const variant = type == 'IN' ? 'ingreso' : 'salida';

    const result = await Swal.fire({
      title: `Esta seguro que quiere marcar ${variant} para la cedula ${form.dni}?`,
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      form.post(route("assistances.store"), {
        onSuccess: () => {
          alerts.success("Registro creado exitosamente");
          form.reset();
        },
      });
    }
  ...
  >
    ...
    <div class="w-6/12 mx-auto sm:px-6 lg:px-8">
      ...
      <div class="flex mt-10 justify-center">
        <h1 class="text-gray-50 uppercase text-2xl">Control de asistencia</h1>
      </div>

      <div class="mt-5">
          <div class="bg-gray-100 border shadow-md rounded px-8 pt-6 pb-8 mb-4">
            <div class="mb-2 h-96">
            ...
            <div class="mb-2">
              ...
              <input
                ...
                type="text"
                v-model="form.dni"
                placeholder="Cedula de empleado"
              />
              <InputError :message="form.errors.dni" class="mt-2"></InputError>
            </div>
           
            <div class="flex items-center justify-around mt-5">
              <PrimaryButton @click="handleSubmit('IN')">
                Entrada
              </PrimaryButton>
              <DangerButton @click="handleSubmit('OUT')">
                Salida
              </DangerButton>
            </div>
          </div>
